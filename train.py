...
camvid_dataset_train = fcn_dataset.CamVidDataset(root='drive/MyDrive/cs585/CS585_spring_A4/CamVid/', images_dir=images_dir_train, labels_dir=labels_dir_train, class_dict_path=class_dict_path, resolution=resolution, crop=True)
dataloader_train = torch.utils.data.DataLoader(camvid_dataset_train, batch_size=batch_size, shuffle=True, num_workers=2)

images_dir_val = "val/"
labels_dir_val = "val_labels/"
camvid_dataset_val = fcn_dataset.CamVidDataset(root='drive/MyDrive/cs585/CS585_spring_A4/CamVid/', images_dir=images_dir_val, labels_dir=labels_dir_val, class_dict_path=class_dict_path, resolution=resolution, crop=False)
dataloader_val = torch.utils.data.DataLoader(camvid_dataset_val, batch_size=1, shuffle=False, num_workers=2, drop_last=False)

images_dir_test = "test/"
labels_dir_test = "test_labels/"
camvid_dataset_test = fcn_dataset.CamVidDataset(root='drive/MyDrive/cs585/CS585_spring_A4/CamVid/', images_dir=images_dir_test, labels_dir=labels_dir_test, class_dict_path=class_dict_path, resolution=resolution, crop=False)
dataloader_test = torch.utils.data.DataLoader(camvid_dataset_test, batch_size=1, shuffle=False, num_workers=2, drop_last=False)
...
def eval_model(model, dataloader, device, num_classes, save_pred=False):
    ...
    with torch.no_grad():
        ...
        for images, labels in dataloader:
            ...
            correct_pixels += predicted.eq(labels).sum().item()
            for i in range(num_classes):
                intersection[i] += ((predicted == i) & (labels == i)).sum().item()
                union[i] += ((predicted == i) | (labels == i)).sum().item()
        pixel_acc = correct_pixels / total_pixels
        # class_iou = np.zeros(num_classes)
        non_zero_indices = np.nonzero(union)
        # class_iou[non_zero_indices] = intersection[non_zero_indices] / union[non_zero_indices]

        # mean_iou = np.mean(class_iou)
        freq_iou = np.sum(intersection) / np.sum(union)
        non_zero_class_iou = intersection[non_zero_indices] / union[non_zero_indices]
        mean_iou = np.mean(non_zero_class_iou)
...
def visualize_model(model, dataloader, device):
    log_dir = "drive/MyDrive/cs585/CS585_spring_A4/vis/"
...
def main():
    # Define the device
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    tensor = torch.randn(16, 3, 384, 512)
    ...
    eval_model(model, dataloader_test, device, num_classes, save_pred=True)
