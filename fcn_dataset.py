    ...
    def parse_class_dict(self, class_dict_path):
        ...
        with open(class_dict_path, 'r') as csvfile:
            ...
            for idx, row in enumerate(csvreader):
                ...
                class_dict[class_id] = (rgb_tuple, class_name)
        return class_dict

    def rgb_to_class_id(self, label_img):
    # Convert an RGB label image to a class ID image (H, W, 3) -> (H, W)
        label_array = np.array(label_img)
        height, width, _ = label_array.shape
        class_id_img = np.zeros((height, width), dtype=np.uint8)

        # Create a dictionary mapping RGB values to class IDs
        rgb_to_class = {rgb: class_id for class_id, (rgb, _) in self.class_dict.items()}

        # Iterate over pixels and map RGB values to class IDs
        for rgb, class_id in rgb_to_class.items():
            mask = np.all(label_array == np.array(rgb), axis=2)
            class_id_img[mask] = class_id

        class_id_img_pil = Image.fromarray(class_id_img)
        return class_id_img_pil
